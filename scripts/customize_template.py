...
class TemplateCustomizer:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"Error: Template configuration file '{self.template_config_path}' not found")
            sys.exit(1)
    ...
    def _validate_variable(self, name: str, value: str) -> bool:
        ...
        if pattern and value and not re.match(pattern, value):
            print(
                f"Error: Variable '{name}' does not match required pattern: {pattern}")
            return False
        ...
        if choices and value and value not in choices:
            print(
                f"Error: Variable '{name}' must be one of: {', '.join(choices)}")
            return False
        ...
        for op in conditional_ops:
            ...
            if self._evaluate_condition(condition, variables):
                if action == 'remove':
                    ...
                    print(
                        f"Manual action required: {op.get('description', 'Modify files')}")
    ...
    def customize(self, args: argparse.Namespace):
        ...
        print(
            f"Your project '{variables.get('APP_NAME', 'Unknown')}' is ready for development.")


def main():
    parser = argparse.ArgumentParser(
        description="Customize qt-simple-template for your project")
    ...
    parser.add_argument('--compiler-toolchain',
                        help='Compiler toolchain preference')

    # Boolean flags
    parser.add_argument('--enable-testing',
                        action='store_true', help='Enable testing')
    parser.add_argument('--disable-testing',
                        action='store_true', help='Disable testing')
    parser.add_argument('--enable-docs', action='store_true',
                        help='Enable documentation')
    parser.add_argument('--disable-docs', action='store_true',
                        help='Disable documentation')
    parser.add_argument('--enable-i18n', action='store_true',
                        help='Enable internationalization')
    parser.add_argument('--disable-i18n', action='store_true',
                        help='Disable internationalization')
    parser.add_argument('--enable-themes',
                        action='store_true', help='Enable themes')
    parser.add_argument('--disable-themes',
                        action='store_true', help='Disable themes')
