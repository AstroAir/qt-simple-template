...
class TemplateValidator:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.errors.append(
                f"Template configuration file '{self.template_config_path}' not found")
            return {}
        ...
        if 'template' in self.config:
            ...
            for field in required_fields:
                if field not in template_info:
                    self.errors.append(
                        f"Missing required field in template section: {field}")
        ...
        if 'templates' in files_config:
            for template_info in files_config['templates']:
                if not isinstance(template_info, dict):
                    self.errors.append(
                        "Template file configuration must be a dictionary")
                    continue
                ...
                if not source or not target:
                    self.errors.append(
                        "Template file must specify both 'source' and 'target'")
                    continue
                ...
                if source_path.exists():
                    ...
                    self._validate_template_file_content(
                        source_path, template_info.get('variables', []))
                elif target_path.exists():
                    # Target exists but no template - this is okay for in-place replacement
                    self._validate_template_file_content(
                        target_path, template_info.get('variables', []))
                else:
                    self.errors.append(
                        f"Neither template source '{source}' nor target '{target}' exists")
        ...
        try:
            ...
            for var in expected_variables:
                if var not in variables_in_file:
                    self.warnings.append(
                        f"Variable '{var}' expected in '{file_path}' but not found")
            ...
            for var in variables_in_file:
                if var not in defined_variables:
                    self.errors.append(
                        f"Undefined variable '{var}' used in '{file_path}'")

        except Exception as e:
            self.errors.append(
                f"Error reading template file '{file_path}': {e}")
        ...
        try:
            ...
            for component in required_components:
                if component not in content:
                    self.warnings.append(
                        f"CMakeLists.txt missing: {component}")
        ...
        try:
            ...
            for field in required_fields:
                if field not in vcpkg_config:
                    self.errors.append(
                        f"vcpkg.json missing required field: {field}")
            ...
            for dep in qt_deps:
                if dep not in [d if isinstance(d, str) else d.get('name') for d in dependencies]:
                    self.warnings.append(
                        f"vcpkg.json missing Qt dependency: {dep}")
    ...
    def validate_documentation_structure(self):
        ...
        expected_doc_dirs = ['api', 'user-guide',
                             'developer-guide', 'template-guide']
        for doc_dir in expected_doc_dirs:
            doc_path = docs_dir / doc_dir
            if not doc_path.is_dir():
                self.warnings.append(
                    f"Documentation directory not found: {doc_path}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate qt-simple-template configuration")
    parser.add_argument(
        '--config', help='Template configuration file', default='template.json')
