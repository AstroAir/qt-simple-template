...
def configure_cmake(preset_name, build_dir):
    ...
    if Path('CMakePresets-extended.json').exists():
        ...
        cmake_cmd = ['cmake', '--preset', preset_name, '--preset-file', preset_file]
    else:
...
def build_project(preset_name, build_dir):
    ...
    if Path('CMakePresets-extended.json').exists():
        cmake_cmd = ['cmake', '--build', '--preset', preset_name, '--preset-file', 'CMakePresets-extended.json']
    else:
...
def main():
    parser = argparse.ArgumentParser(description="Build qt-simple-template with Conan")
