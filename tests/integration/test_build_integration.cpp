...
void TestBuildIntegration::testConfigurationGeneration()
{
    ...
    for (const QString& part : versionParts) {
        part.toInt(&ok);
        QVERIFY2(ok, qPrintable(QString("Version part '%1' is not numeric").arg(part)));
    }
}
...
void TestBuildIntegration::testResourceCompilation()
{
    ...
    QVERIFY2(resourceDir.exists(), "Light theme resources should be accessible");
    
    QDir darkResourceDir(":/images/dark");
    QVERIFY2(darkResourceDir.exists(), "Dark theme resources should be accessible");
    
    // Test specific resource files
    QFile lightThemeImage(":/images/light/theme");
    QVERIFY2(lightThemeImage.exists(), "Light theme image should be accessible");
...
void TestBuildIntegration::testDependencyLinking()
{
    ...
    QVERIFY(true); // Placeholder - actual linking is verified by successful build
}
...
void TestBuildIntegration::testExecutableProperties()
{
    ...
    if (!stylesDir.exists()) {
        ...
        qDebug() << "Styles directory not found at:" << stylesDir.absolutePath();
