...
void BaseView::setController(IController *controller) {
    ...
    if (m_controller) {
        connect(m_controller, &IController::stateChanged, this,
                &BaseView::onControllerStateChanged);
        connect(m_controller, &IController::errorOccurred, this,
                &BaseView::showError);
    }
}

IController *BaseView::getController() const { return m_controller; }
...
void BaseView::setViewEnabled(bool enabled) { setEnabled(enabled); }

bool BaseView::isViewValid() const { return m_initialized && validateView(); }
...
QVBoxLayout *BaseView::getMainLayout() const { return m_mainLayout; }
...
void BaseView::addSeparator() {
    ...
    m_mainLayout->insertWidget(m_mainLayout->count() - 2,
                               separator);  // Insert before stretch and status
}
...
void BaseView::onControllerStateChanged() { updateView(); }
